...
void HumanDynamicsEstimator::run()
{
    ...
    for (const iDynTree::BerdySensor& task1Sensor : task1BerdySensors) {
        ...
        if (pImpl->task1SensorMapIndex.find(key) != pImpl->task1SensorMapIndex.end()) {
            ...
            switch (task1Sensor.type)
            {
                ...
                {
                    // Multiply with offset removed mass
                    yInfo() << LogPrefix << "Total mass is " << pImpl->humanModel.getTotalMass() << " offset mass is " << pImpl->offsetMass;
                    pImpl->berdyData.buffers.task1_measurements(found->second.offset + 0) = (pImpl->humanModel.getTotalMass() - pImpl->offsetMass) * comProperAccelerationInBaseFrame[0];
                    pImpl->berdyData.buffers.task1_measurements(found->second.offset + 1) = (pImpl->humanModel.getTotalMass() - pImpl->offsetMass) * comProperAccelerationInBaseFrame[1];
                    pImpl->berdyData.buffers.task1_measurements(found->second.offset + 2) = (pImpl->humanModel.getTotalMass() - pImpl->offsetMass) * comProperAccelerationInBaseFrame[2];
                    pImpl->berdyData.buffers.task1_measurements(found->second.offset + 3) = (pImpl->humanModel.getTotalMass() - pImpl->offsetMass) * comProperAccelerationInBaseFrame[3];
                    pImpl->berdyData.buffers.task1_measurements(found->second.offset + 4) = (pImpl->humanModel.getTotalMass() - pImpl->offsetMass) * comProperAccelerationInBaseFrame[4];
                    pImpl->berdyData.buffers.task1_measurements(found->second.offset + 5) = (pImpl->humanModel.getTotalMass() - pImpl->offsetMass) * comProperAccelerationInBaseFrame[5];
                }
                break;
    ...
    if (pImpl->commandPro->cmdOffsetStatus && !pImpl->commandPro->resetOffset) {
        if (pImpl->removeOffsetOption == "model") {

            // Variable for total offset wrench from dummy sources
            iDynTree::Wrench totalOffsetWrench;
            totalOffsetWrench.zero();
            ...
            for (auto& element : pImpl->wrenchSourceNameAndType) {
                ...
                if (element.second == WrenchSourceType::Dummy) {
                    ...
                    // Get link wrench estimate
                    iDynTree::Wrench linkWrenchEstimateLinkFrame = pImpl->berdyData.estimates.task1_linkNetExternalWrenchEstimates(linkIndex);

                    // Transform link wrench esitmate to world frame
                    iDynTree::Wrench linkWrenchEstimateWorldFrame;
                    linkWrenchEstimateWorldFrame.zero();

                    Eigen::Matrix<double,6,1> transformedWrenchEigen = iDynTree::toEigen(world_H_link.asAdjointTransformWrench())
                                                                       * iDynTree::toEigen(linkWrenchEstimateLinkFrame.asVector());
                    iDynTree::fromEigen(linkWrenchEstimateWorldFrame, transformedWrenchEigen);


                    // Sum the wrench estimates from dummy sources expressed in world orientation
                    totalOffsetWrench = totalOffsetWrench + linkWrenchEstimateWorldFrame;

                }
            }

            // Compute the offset mass related to the wrench estiamtes at dummy sources
            double norm = std::sqrt( std::pow(totalOffsetWrench.getVal(0), 2) +
                                     std::pow(totalOffsetWrench.getVal(1), 2) +
                                     std::pow(totalOffsetWrench.getVal(2), 2) +
                                     std::pow(totalOffsetWrench.getVal(3), 2) +
                                     std::pow(totalOffsetWrench.getVal(4), 2) +
                                     std::pow(totalOffsetWrench.getVal(5), 2));

            pImpl->offsetMass = norm/std::sqrt(std::pow(pImpl->gravity(0), 2) +
                                               std::pow(pImpl->gravity(1), 2) +
                                               std::pow(pImpl->gravity(2), 2));
